...
description: Run the reviewed TokenMonster Electron source-development launch, status, or stop workflow. Use only when the user explicitly asks to operate TokenMonster from the checked-out repository instead of installing or releasing it.
---
...
completely before running a command. Treat that document and `AGENTS.md` as the
authority.

## Select the operation

- Start or restart only after an explicit user request. If the request is not
  explicit, explain the available operation and wait.
