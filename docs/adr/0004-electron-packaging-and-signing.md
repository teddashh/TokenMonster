...
- Forge 7.11.2 still declares `@electron/rebuild ^3.7.0`, whose resolved tree
  ...
  versions or an equally reviewed upstream fix replaces the overrides.
...
internal. Signed mode is macOS-only and fails before packaging unless all of
the following are present and structurally valid:
