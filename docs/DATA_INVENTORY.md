...
assets. On Windows, only fixed content-blind startup phase names and sanitized
terminal failure names may join that safe log; the ephemeral readiness-pipe
identifier, capability, and raw `HELLO`/phase/failure/readiness bytes are never
logged or persisted.
