...
- `packages/monster-engine` 純函式輸入／輸出：
  ...
  - 產出 `characterId`、2–3 個 dominant traits、mood、evolution cadence、explanation keys 與顯示用安全數值；
...
- Local share-card renderer：卡片包含 portrait、2–3 個 trait、歸因句、期間與可選的 contributor counter；預設下載本機圖片，不建立公開 URL。
...
stop-abort與wake／overlap本機tests，但server pause/resume仍未實作。D1 `day-all-v1` compactor及Worker
