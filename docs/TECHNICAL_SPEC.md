...
8. Release 內嵌 image-only strict schema-v2 rights authority、descriptor 與 exact HTTPS allowlist，涵蓋 11 位角色的 891 張圖片與 0 條語音。候選 npm package 在 release staging 另加入該 authority 的精確 8 個 WebP／415,470 bytes：ChatGPT、Claude、Gemini、Grok 各一張 avatar 與 `tech` 基本服裝，搭配 168 條 `zh-TW`／`en` 內建文字且不含 audio。未同意、離線缺完整 cache、失敗或撤銷時，四位基本圖文仍為 zero runtime GET，其他缺圖狀態才 letter/silent fallback；明確啟用才單次取得一個 891-image usage-independent fixed pack。Schema-v1 manifest 只保留為歷史 audit input，其名稱不代表 public approval。這些是候選 artifact 規則，不代表 application release 已發布。
...
- AI-Sister source 與 raw parts 不 vendoring 或 submodule 進 TokenMonster。TokenMonster release 只內嵌通過 schema-v2 gate 的 immutable output manifest、固定 pack descriptor、exact allowlist 與必要 persona facts；目前 image-only fixed pack 由 AI-Sister 管理的 `https://cdn.ted-h.com` 提供，只有使用者明確啟用才發出一個固定 GET。
...
歷史 schema-v1 integrity input 列出 10 位角色、各 20 種衣櫥與 pose art（810 image refs），以及每人五條、共 50 條 prerecorded WAV refs；它不再是 release authority。Current v2 image release 另以完整 source evidence、provenance、rights／brand／content review與中英文 alt text 核准 11 個 avatars、220 個 outfits 與 660 個 poses。歷史 50 條 WAV 是 voice-clone outputs；即使 owner approval 已存成私有憑證，現有 schema 仍不接受把它們重標成 `human-original` 或 `synthetic-non-clone`，而且其 speaker/clone consent chain、逐 clip spoken-content review與 metadata-stripping evidence不完整。因此目前 authority 明確是 0 voice；語音須完成技術證據與 deterministic metadata removal後，以新的 image + voice 合併 immutable release 發布，不能用 voice-only slot 取代現有圖片 authority。
...
- `@tokenmonster/characters/asset-pack` 提供已接線的 fixed-pack 驗證/cache primitive：完整 pack hash 先驗、strict classic ZIP profile、固定 entry set/order、redirect/timeout/transfer/extraction caps、逐 entry bytes/media type/SHA-256 與 atomic mode-0600 cache。Current descriptor只允許一個含891張圖片、65,574,180 bytes 的固定 ZIP；候選 npm artifact 只含精確 8 個基本 WebP，且完全不含音訊 binary。
- 使用者充分揭露後明確啟用才可取得完整、固定、versioned pack；網路物件集合與次序不得依任何本機用量、角色或解鎖狀態改變。四位 starter avatar、`tech` 基本服裝與文字選擇可直接使用內嵌 base；其他 theme／pose 選擇只可在完整 pack 已於本機驗證後發生。新語音也必須先進入新的 eligible combined pack，不能由 trigger 另行下載。
...
Companion 已有 default-off 的本機預錄 voice playback capability；它只對 unlocked character 暴露 strict audio-manifest lines，且只播放逐次驗證的 cache 命中，不會依 trigger 連網。目前 release-only starter base 與 891-image pack 都明確含 0 voice，因此一般行為仍是靜音。四位 starter 的 168 條 `zh-TW`／`en` fixed lines 是內建文字，不是音訊。Runtime 能播放不等於 voice evidence 或 fixed-pack privacy gate 已完成。後續 online TTS 必須新增獨立 consent、provider adapter、keychain credential 與 data-retention disclosure；請求仍由 companion 直接送 provider，中央不 proxy。文字/音訊只在 RAM 或明確的使用者匯出路徑，預設不持久化。

OS built-in TTS 若啟用，必須先核對平台散布/錄製條款，預設關閉。歷史 50 條 cloned WAV 的 owner approval 已私下保存，但不能替代 speaker/clone consent provenance、逐 clip content review或 metadata-stripping evidence；在這些技術條件完成前不得發布或描述成 current pack capability。每個 voice set 的證據仍需與 image asset 分開審查，但 runtime publication 必須產生新的 combined image + voice authority／descriptor／pack，避免 voice-only authority 讓現有圖片消失。
