...
截至 2026-07-16，local Companion、exact-pinned TokenTracker sidecar、11 位角色／進度、
按需 asset downloader／cache、BYOK/固定互動、monster、Web/API、
...
8. Release 內嵌 10 位角色的 approved strict asset manifest，GLM 使用 letter mode。圖像與後續語音不進入 npm package，只能經 manifest rights gate 後從固定 CDN 按需取得。
...
目前 release-embedded strict manifest 列出 10 位角色、各 20 種衣櫥與 pose art；GLM 不在圖像 manifest 內並使用 code-native letter mode。Manifest 的 `voice` 目前為空；real-speaker reference 與 generated/cloned voices 不得進入 release。新圖像或語音仍必須通過獨立 rights／brand gate 才能加入 manifest。
...
- Downloader 只依內嵌 manifest 在解鎖後按需抓取 hash-named object，拒絕 redirect，套用 10 秒 timeout、4 MiB response cap 與 SHA-256 驗證，再 atomic 寫入 `~/.tokenmonster/asset-cache`；cache hit 也重新驗證 hash。任何離線、404、timeout、hash mismatch 或 cache corruption 都回退到內建 letter renderer，且不影響本機 usage 功能。
- `--no-character-downloads` 將 CDN origin 設為 null，使圖像與後續語音的 network fetch 完全停用；已驗證 cache 與 letter fallback 仍可用。
